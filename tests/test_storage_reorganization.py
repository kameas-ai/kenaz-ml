...
* **NFR-003** -- the two new re-exporting ``__init__.py`` files do not make
  ``import kenaz_ml.app`` materially more expensive.

Measurements taken for WP03 (Python 3.12.13, this hardware)
-----------------------------------------------------------
``python -X importtime -c "import kenaz_ml.app"`` cumulative, 5 runs:
...
new packages account for 744 us of that total (``kenaz_ml.datastore`` 186 us
cumulative, ``kenaz_ml.modelstore`` 558 us), of which the re-exporting
...
import kenaz_ml
import kenaz_ml.datastore
import kenaz_ml.modelstore
from kenaz_ml.datastore import DataStore, create_store
from kenaz_ml.modelstore import (
    CachedModelStore,
...
# disappeared entirely. quickstart.md §2 lists six; ``kenaz_ml.storage`` is
# included as a seventh because D-004 removes the package, not just the
# module inside it.
OLD_MODULE_PATHS = (
    "kenaz_ml.store",
    "kenaz_ml.store_sqlite",
    "kenaz_ml.store_postgres",
    "kenaz_ml.storage",
    "kenaz_ml.storage.model_store",
    "kenaz_ml.loader",
    "kenaz_ml.cache",
)
...
def _package_root() -> Path:
    """The installed ``kenaz_ml/`` directory."""
    assert kenaz_ml.__file__ is not None
    return Path(kenaz_ml.__file__).parent
...
class TestOldPathsAreGone:
    ...
    def test_the_storage_package_directory_is_gone(self) -> None:
        """D-004: ``kenaz_ml/storage/`` disappears entirely, shim included."""
...
class TestPublicSurfaceResolvesFromThePackage:
    ...
    def test_datastore_name_is_a_package_attribute(self, name: str) -> None:
        assert hasattr(kenaz_ml.datastore, name), f"kenaz_ml.datastore has no attribute {name!r}"

    @pytest.mark.parametrize("name", MODELSTORE_SURFACE)
    def test_modelstore_name_is_a_package_attribute(self, name: str) -> None:
        assert hasattr(kenaz_ml.modelstore, name), f"kenaz_ml.modelstore has no attribute {name!r}"
    ...
    def test_the_plan_snippet_imports_verbatim(self) -> None:
        ...
        for name, obj in bound.items():
            assert getattr(kenaz_ml.datastore, name) is obj
        ...
        for name, obj in bound.items():
            assert getattr(kenaz_ml.modelstore, name) is obj

    @pytest.mark.parametrize(
        ("package", "submodule", "names"),
        [
            ("kenaz_ml.datastore", "kenaz_ml.datastore.protocol", DATASTORE_SURFACE),
            (
                "kenaz_ml.modelstore",
                "kenaz_ml.modelstore.stores",
                ("ModelStore", "LocalModelStore", "S3ModelStore", "CachedModelStore", "model_store_factory"),
            ),
            ("kenaz_ml.modelstore", "kenaz_ml.modelstore.loader", ("ModelLoader", "FilesystemModelLoader")),
            ("kenaz_ml.modelstore", "kenaz_ml.modelstore.cache", ("ModelCache", "create_model_cache")),
        ],
    )
    ...
    def test_submodule_paths_remain_importable(self) -> None:
        ...
        """
        for submodule in (
            "kenaz_ml.datastore.protocol",
            "kenaz_ml.datastore.sqlite",
            "kenaz_ml.modelstore.stores",
            "kenaz_ml.modelstore.loader",
            "kenaz_ml.modelstore.cache",
        ):
...
class TestFactoryNameWasNotInvented:
    ...
    def test_model_store_factory_is_exported(self) -> None:
        ...
        assert "model_store_factory" in kenaz_ml.modelstore.__all__

    def test_create_model_store_does_not_exist(self) -> None:
        assert not hasattr(kenaz_ml.modelstore, "create_model_store")
        assert "create_model_store" not in kenaz_ml.modelstore.__all__
...
class TestAllMatchesTheImportableSurface:
    ...
    @pytest.mark.parametrize("package", ["kenaz_ml.datastore", "kenaz_ml.modelstore"])
    ...
    @pytest.mark.parametrize(
        ("package", "expected"),
        [
            ("kenaz_ml.datastore", set(DATASTORE_SURFACE)),
            ("kenaz_ml.modelstore", set(MODELSTORE_SURFACE)),
        ],
    )
...
class TestImportCostOfTheNewPackages:
    ...
    the share of ``import kenaz_ml.app`` that the two new packages account
    for. That share is what two extra ``__init__.py`` files could plausibly
    inflate, and it is hardware-independent.
    """

    NEW_PACKAGES = ("kenaz_ml.datastore", "kenaz_ml.modelstore")
    ...
    def _importtime_cumulative() -> dict[str, int]:
        ...
        proc = subprocess.run(
            [sys.executable, "-X", "importtime", "-c", "import kenaz_ml.app"],
            capture_output=True,
            text=True,
            check=True,
        )
    ...
    def test_the_new_packages_are_a_negligible_share_of_import_time(self) -> None:
        ...
        assert "kenaz_ml.app" in cumulative, "importtime output did not name kenaz_ml.app"
        total = cumulative["kenaz_ml.app"]
        assert total > 0

        measured = {name: cumulative.get(name, 0) for name in self.NEW_PACKAGES}
        assert all(value > 0 for value in measured.values()), (
            f"expected both new packages on the import path of kenaz_ml.app, got {measured}"
        )

        share = sum(measured.values()) / total
        assert share < self.BUDGET_SHARE, (
            f"new storage packages cost {share:.2%} of `import kenaz_ml.app` "
    ...
    def test_the_postgres_backend_is_not_imported_eagerly(self) -> None:
        ...
        ``kenaz_ml.app`` would drag the optional driver into the local
        deployment and into the frozen bundle's signable surface.
        """
        cumulative = self._importtime_cumulative()
        assert "kenaz_ml.datastore.postgres" not in cumulative
