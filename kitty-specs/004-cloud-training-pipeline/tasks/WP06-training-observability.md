...
- **Output strategy**: Structured JSON goes to stdout; log/diagnostic messages go to stderr. This allows piping: `kenaz-ml train ... | jq .` while still seeing operational logs.
...
- **Validation**:
  ...
  - [ ] Compact when piped: `kenaz-ml train ... | jq .`
...
- **Valid JSON**: Run `kenaz-ml train --mode cloud --all-tenants | jq .` and verify parseable output.
