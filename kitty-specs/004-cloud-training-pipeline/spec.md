...
1. **Given** 10 tenants exist with synced data, **When** `kenaz-ml train --mode cloud --all-tenants` is run, **Then** each eligible tenant is trained and a summary report is produced listing trained/skipped/failed tenants.
...
1. **Given** 5 tenants have opted in to data pooling, **When** `kenaz-ml train --mode cloud --aggregate` is run, **Then** events from all 5 tenants are combined for training and aggregate weights are saved to a shared storage prefix.
...
A free-tier developer runs `kenaz-ml train` locally. The existing training behavior — reading from SQLite, training all models, saving `.joblib` files locally — works identically. The background training scheduler continues to function.

**Why this priority**: Local training must not regress. Cloud training is additive.

**Independent Test**: Run `kenaz-ml train` without cloud flags. Verify models train from SQLite and save to the local models directory.

**Acceptance Scenarios**:

1. **Given** kenaz-ml is in local mode, **When** `kenaz-ml train` is run, **Then** behavior is identical to current implementation.
2. **Given** kenaz-ml is in local mode, **When** the background training scheduler triggers, **Then** it retrains from SQLite and reloads models into the running poller.
...
- **FR-001**: System MUST support a `kenaz-ml train --mode cloud --tenant <id>` command that trains all models for a single tenant from Postgres data.
- **FR-002**: System MUST support a `kenaz-ml train --mode cloud --all-tenants` command that discovers and trains all eligible tenants in batch.
- **FR-003**: System MUST support a `kenaz-ml train --mode cloud --aggregate` command that trains aggregate models from pooled opted-in tenant data.
...
- **FR-011**: The existing local training command (`kenaz-ml train`) and background scheduler MUST continue to work without modification.
...
- **SC-004**: Local training (`kenaz-ml train`) passes all existing tests with no regressions after cloud training changes.
...
- **External: Postgres with synced data**: Training requires events and tasks synced from user laptops via the sync agent (sigild feature, not kenaz-ml).
- **External: S3 bucket**: Model weights are persisted to S3. Must be accessible from the K8s cluster.
- **External: K8s CronJob**: The training entrypoint is invoked as a CronJob. K8s scheduling is configured by the operator, not by kenaz-ml.
