...
A structured event emitted when a model detects something noteworthy. Written by kenaz-ml, read by sigild.
...
Not a new table — extends existing `suggestions` table with `signal_id` column (implemented in sigil Feature 021). kenaz-ml reads the linkage for training labels:
