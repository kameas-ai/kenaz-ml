...
- CLI `kenaz-ml train` creates the correct `ModelStore` and passes to `Trainer`.
     ...
     async def startup_event() -> None:
         ...
         try:
             ...
             logger.error(
                 ...
                 "kenaz-ml[cloud] is installed for cloud mode.",
                 backend, e,
             )
         ...
         logger.info(
             "kenaz-ml: mode=%s, backend=%s, models loaded, poller started",
...
- **Purpose**: The `kenaz-ml train` CLI command must create the appropriate `ModelStore` via factory and pass it to `Trainer`, matching the server's behavior.
