...
if TYPE_CHECKING:  # pragma: no cover - typing only
    from sigil_ml.modelstore.registry import FeatureContract, Manifest, Refusal

logger = logging.getLogger(__name__)

#: The sidecar pair's file extensions. Restated rather than imported from
#: `sigil_ml.modelstore.registry`, because every import in this module is
#: deliberately deferred into the function that needs it: `sigil_ml.modelstore`
#: is imported by `sigil_ml.app` on every start, and the registry is only
...
def _migrate_pre_registry(slot_dir: Path, model_name: str, contract: FeatureContract | None) -> Manifest | None:
    ...
    """
    from sigil_ml.modelstore.registry import write_manifest
...
def _validated_load(
    ...
    """
    from sigil_ml.modelstore.registry import deserialize_verified, validate_artifact
...
class FilesystemModelLoader:
    ...
    def _load(self, tenant_id: str, model_name: str) -> Any | None:
        from sigil_ml.modelstore.registry import REASON_SLOT_EMPTY, resolve_model
