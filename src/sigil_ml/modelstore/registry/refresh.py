...
:class:`~sigil_ml.modelstore.registry.manifest.Refusal` as data and leaves the
...
#: entry — :func:`~sigil_ml.modelstore.registry.slots.resolve_model` looks for
    ...
    * **Neither present.** Falls out of the first check as ``no_base``.
    ...
    Args:
        ...
        local_dir: Overrides the local slot; defaults to
            :func:`~sigil_ml.modelstore.registry.slots.local_slot_dir`.
        base_dir: Overrides the base slot; defaults to
            :func:`~sigil_ml.modelstore.registry.slots.base_slot_dir`.
...
def _peek_next_generation(current: str | None) -> str:
    ...
    """
    from sigil_ml.modelstore.registry.retained import next_generation
