"""Feast entities, feature views, and feature services for ``sigil_ml``.
...
**No feature arithmetic lives here** (plan.md D-002). ``sigil_ml.features``
...
def _schema(entity: Entity, feature_names: list[str]) -> list[Field]:
    ...
    Every feature is ``Float64`` — the extractors in ``sigil_ml.features`` return
...
# `sigil_ml.models` and fails if that ever happens.
#
