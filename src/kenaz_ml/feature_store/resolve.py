...
``kenaz_ml.features`` and returned from that computation; the online store is
...
# Nothing from `feast`, `pandas` or `kenaz_ml.feature_store.definitions` is
...
class FeatureFamily:
    ...
    from :mod:`kenaz_ml.feature_store.definitions` so that importing this module
...
#: syntax.
_active_resolution: contextvars.ContextVar[bool] = contextvars.ContextVar("kenaz_ml_active_resolution", default=False)
...
def resolve_stuck_features(store: DataStore, task_id: str) -> dict[str, float]:
    ...
    Returns:
        Exactly what ``kenaz_ml.features.extract_stuck_features`` returns — same
...
def resolve_stuck_features_from_buffer(events: list[dict]) -> dict[str, float]:
    ...
    Returns:
        Exactly what ``kenaz_ml.features.extract_features_from_buffer`` returns.
    """
    token = _active_resolution.set(True)
    try:
...
def read_online_features(family: FeatureFamily, entity_id: str) -> dict[str, float | None]:
    ...
    *push* loses a byproduct the next cycle recreates, while a failed *read* is
    ...
    from kenaz_ml.feature_store.definitions import REGISTERED_FEATURE_NAMES
...
def _feature_store() -> Any:
    ...
    try:
        ...
        from kenaz_ml.feature_store.config import load_repo_config
