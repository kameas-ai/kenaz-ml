...
:func:`~kenaz_ml.training.trainer._reference_time_for` the training path uses —
...
class PostgresOfflineSink:
    ...
    :mod:`kenaz_ml.feature_store.config`, and a module that opens its own socket
        ...
        for view in selected:
            ...
            result.rows.append(
                OfflineFeatureRow(
                    ...
                    # are whatever `kenaz_ml.features` returned, coerced only to
...
def materialize_tasks(
    ...
    *,
    ...
    Args:
        data_store: A :class:`~kenaz_ml.datastore.DataStore`. Reads only.
...
def postgres_source(view: OfflineFeatureView) -> Any:
    ...
    :func:`kenaz_ml.feature_store.config.load_cloud_repo_config`, which derives
    them from the same ``KENAZ_POSTGRES_URL`` ``PostgresStore`` already uses, so
...
def feature_store_for_cloud() -> Any:
    ...
    from kenaz_ml.feature_store.config import load_cloud_repo_config
