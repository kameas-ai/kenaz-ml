...
see, based on :func:`kenaz_ml.config.operating_mode`, the same switch that
...
from kenaz_ml import config as kenaz_config
...
def bundle_dir() -> Path:
    ...
    if meipass is not None:
        return Path(meipass) / "kenaz_ml" / "feature_store"
...
def user_data_dir() -> Path:
    ...
    Derived from :func:`kenaz_ml.config.db_path` rather than recomputed, so the
    online store cannot drift away from the shared database it sits beside if
    ``XDG_DATA_HOME`` moves.
    """
    return kenaz_config.db_path().parent
...
def cloud_config_path() -> Path:
    ...
    """
    mode = kenaz_config.operating_mode()
    if mode != "cloud":
        raise CloudConfigUnreachableError(
            ...
            "if you meant to run the cloud deployment, set KENAZ_MODE=cloud."
        )
...
def _postgres_connection() -> dict[str, Any]:
    ...
    Read from :func:`kenaz_ml.config.postgres_url`, the same variable
    ...
    """
    url = kenaz_config.postgres_url()
    if not url:
        raise RuntimeError(
            "KENAZ_POSTGRES_URL is not set. The cloud feature store needs it for both the "
            "offline and the online store (FR-006)."
        )
    parts = urlparse(url)
    database = parts.path.lstrip("/")
    if not database:
        raise RuntimeError(f"KENAZ_POSTGRES_URL names no database: {parts.scheme}://.../<database>")
...
def load_cloud_repo_config() -> Any:
    ...
    parsed["registry"] = {**parsed.get("registry", {}), "path": kenaz_config.postgres_url()}
