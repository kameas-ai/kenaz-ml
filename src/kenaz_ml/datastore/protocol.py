"""DataStore protocol — the central abstraction for all data access in kenaz-ml.

All modules (poller, routes, trainer, scheduler) depend on this protocol
instead of importing sqlite3 or psycopg2 directly.

Implementations:
    SqliteStore  (local mode)  — src/kenaz_ml/datastore/sqlite.py
    PostgresStore (cloud mode) — src/kenaz_ml/datastore/postgres.py
"""
...
def create_store(mode: str | None = None) -> DataStore:
    ...
    if resolved_mode == "cloud":
        ...
        if not url:
            raise ValueError("KENAZ_POSTGRES_URL environment variable is required in cloud mode")
