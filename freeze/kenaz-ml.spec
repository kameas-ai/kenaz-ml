...
KENAZ_ML_SRC = REPO_ROOT / "src" / "kenaz_ml"
FEATURE_STORE_SRC = KENAZ_ML_SRC / "feature_store"

# Where the collected feature-store assets must land inside the bundle. This is
# not a free choice: `kenaz_ml.feature_store.config.bundle_dir()` resolves to
# `<sys._MEIPASS>/kenaz_ml/feature_store` when frozen, and every path in the
...
BUNDLE_FEATURE_STORE_DIR = os.path.join("kenaz_ml", "feature_store")
...
def _apply_registry(staging_dir):
    ...
    `kenaz_ml.feature_store.config`. The API path uses exactly the shipped
    ...
    * a throwaway temp directory stands in for the writable user data
    ...
    from kenaz_ml.feature_store import definitions as fs_definitions
    ...
    try:
        from kenaz_ml.feature_store.resolve import local_feature_views
        ...
        print(
            "[kenaz-ml.spec] WARNING: kenaz_ml.feature_store.resolve.local_feature_views is not "
    ...
    print(
        f"[kenaz-ml.spec] feast apply OK — registry {registry} "
        f"({registry.stat().st_size} bytes) produced by feast {feast.__version__}"
    )
    return registry, marker


# An editable install already puts `kenaz_ml` on the path; a plain source
...
hiddenimports += [
    ...
    "uvicorn.lifespan",
    "uvicorn.lifespan.on",
    # The app is loaded by uvicorn via the string "kenaz_ml.app:app", so the
    # whole package must be reachable in the frozen graph.
    "kenaz_ml.app",
]
hiddenimports += collect_submodules("kenaz_ml")
...
# `kenaz_ml`, and pyarrow in particular loads its compiled `_*.so` modules
